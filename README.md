...
#### 李大双
- **论文名字** : Effective Decoding in Graph Auto-Encoder Using Triadic Closure ;
- **论文来源** : 2020AAAI ;
- **代码地址** : [连接](None) 或者 None ;
- **方法简称** : TGA ;
- **方法方向** : 图自编码器 ;
- **快速获取** :
```powershell
...
| 方法介绍 |
| :-------- |
| （变异）图形自动编码器及其变体已普遍用于图形结构化数据的表示学习。虽然编码器通常是一个强大的图形卷积网络，但解码器通过一次只考虑两个节点来重建图形结构，从而忽略边缘之间可能的交互。另一方面，同时考虑整个图形的结构化预测的计算成本很高。本文利用在许多真实网络中展示的众所周知的三合会封闭属性。我们提出三合会解码器，它共同考虑和预测本地三合会所涉及的三个边缘。三合会解码器可以很容易地用于任何基于图形的自动编码器。特别是，我们将它合并到（变异）图形自动编码器中。对链路预测、节点聚类和图形生成的实验表明，三合会的使用可以更准确地预测、聚类，更好地保存图形特征。|
